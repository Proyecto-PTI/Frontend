...
const Users = () => {
  ...
  const handleSearch = (e) => {
    ...
    setFiltered(users.filter(user =>
        user.metadatos.id_persona.toLowerCase().includes(term) ||
        user.metadatos.email.toLowerCase().includes(term) ||
        user.metadatos.user_role.toLowerCase().includes(term)
    ));
  };
          ...
          <div className={styles.userList}>
            {filtered.map(user => (
                <UserCard
                    key={user.id}
                    id={user.id}
                    name={user.metadatos.id_persona} // Accediendo a `id_persona` dentro de `metadatos`
                    email={user.metadatos.email} // Accediendo a `email` dentro de `metadatos`
                    role={user.metadatos.user_role} // Accediendo a `user_role` dentro de `metadatos`
                />
            ))}
          </div>
        </div>
