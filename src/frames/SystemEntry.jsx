...
function SystemEntry() {
    const { aula } = useParams(); // Obtén el aula de la URL
    ...
    const handleSelectLog = (log) => {
        console.log("Log clicado:", log); // Verifica si el log clicado llega aquí
        setSelectedLog(log); // Actualiza el estado con el log seleccionado
        setEditableAttributes({ ...log }); // Actualiza los atributos editables con los datos del log
        setEditingField(null); // Sal de cualquier modo de edición
    };

    // Save changes to the selected log
    ...
    return (
        <div className={styles.frameContainer}>
            ...
            <div className={styles.container}>
                ...
                {/* Latest Access Logs */}
                <Access
                    logs={logs} // Usa los logs obtenidos del backend
                    usernameFilter=""
                    statusFilter=""
                    codeFilter={aula} // Filtra por el aula seleccionada
                    dateFilter=""
                    onLogClick={(log) => handleSelectLog(log)}
                />
            </div>
        </div>
    );
