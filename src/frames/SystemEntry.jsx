...
function SystemEntry() {
    const { id } = useParams();
    const [logs, setLogs] = useState([]); // Cambié logs aquí
    ...
    const handleSaveChanges = async () => {
        ...
        try {
            const response = await fetch(`http://localhost:8000/entries/${editableAttributes.id}`, {
                method: 'PATCH',
                headers: {
            ...
            setLogs((prevLogs) =>
                prevLogs.map((log) =>
                    log.code === data.entry.code ? data.entry : log
                )
            );
    ...
    const fetchRecentLogs = async () => {
      try {
        const response = await fetch(`http://localhost:8000/entrylogs/${id}`);
        const data = await response.json();
    ...
    return (
        <div className={styles.frameContainer}>
            ...
            <div className={styles.container}>
                <WebHeader subtitle={`View System Entry for ${editableAttributes.code || id}`} />
                <div>
                ...
                <div className={styles.content}>
                    {selectedLog && (
                        <div className={styles.entryDetails}>
                            ...
                            {["location", "cameraid", "lockid", "administrator"].map((attribute) => (
                    ...
                    {selectedLog && (
                        ...
                        initialLabels={editableAttributes.labels || []}
